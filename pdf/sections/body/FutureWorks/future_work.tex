...
\todo{switch has never been mentioned before this point}
...
The next two lines in Figure~\ref{figure:future_work:switch_out}, those beginning with \textit{signal}, show the data of the live signals $c$ and $s$.
...
\subsection{Native implementations of primitive combinators}

Most signal combinators are implemented in\ \todo{RizzoLang} itself. While this is safe it does mean that some combinators allocate more than is really necessary.
...
Every (recursive) call therefore allocates a fresh \textbf{ctor} value for the $\mathsf{laterapp}$, a \textbf{ctor} value for $\mathsf{delay}$, and a thunk for the lambda expression.

Since the shape of the values generated by $mk\_sig$ never change, it means when a signal built with $mk\_sig$ is advanced, the tail of the signal is reconstructed as a new value that is structurally identical to the old one. In other words, the runtime repeatedly allocates a fresh copy of the same tail, only to discard the previous copy immediately afterwards. This is wasted work.
A native implementation of $mk\_sig$ may be able to `cheat' by keeping track of the later and then advancing it in place.
\todo{Not sure how to solve the issue? Do we need to be more precise?}
