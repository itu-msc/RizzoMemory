...
Signals are updated signals in place whenever the environment produces a value that is relevant for the signal. These updates are done, not by the programmer, but by the reactive semantics of Rizzo, see Section~\ref{section:frp:rizzo}. For convenience we define the signal constructor as an infix operator: 
...
Another later constructor is the $\text{tail} : \texttt{Signal }a \rightarrow \texttt{Later }(\texttt{Signal }a)$, which let us access the delayed computation for the future value of a signal. 
...
This allows later values to be applied to delayed functions but the application does not happen until a future time step when the argument $x$ has become available.

\todo{Figure with later modality types?}

With these constructors we already have the necessary tools to define some signal combinators! 
The $mk\_sig$ function allows us to create signals from any delayed computation:

\[ \text{mk\_sig} \; later = (delay \; (\lambda x. x :: \text{mk\_sig }later)) \; \triangleright \; later \]
...
We can take it one step further and define the function $\text{map} : (a \rightarrow b) \rightarrow \texttt{Signal }a \rightarrow \texttt{Signal }b$:

\[ \text{map} \; f \; (x :: xs) = f \; x :: (delay \; (map \; f)) \triangleright xs \]

The definition of map makes use of pattern matching on signals. We could alternatively used explicit $\text{head}$ and $\text{tail}$ to extract $x$ and $xs$ respectively.

Let us return to what makes Rizzo different, its semantics of signals. 
Because signals are updated in place as mutable references, as opposed to stored as immutable sequences, is it possible to define combinators like $\text{sample} : \texttt{Signal }a \rightarrow \texttt{Signal }b \rightarrow \texttt{Signal } (a \times b)$:
...
In Rizzo, time will step forward whenever a value is produced by a channel. This could be as a result of a message from a network socket, a sensor reading, or a user entering something into the console. Time steps are discrete, meaning they happen one-after-the-other and never at the same time. This is important because whenever time advances, Rizzo must bring all signals into this new time step by updating them. In order for signals to be reachable during update, they are all stored in a signal-only heap. The heap is structure as a list because the ordering is important for making sure signals are updated in the correct order.
