...
Let us return to what makes Rizzo different, its semantics of signals. Because signals are updated in place as mutable references, as opposed to stored as immutable sequences, is it possible to define combinators like $\text{sample} : \texttt{Signal }a \rightarrow \texttt{Signal }b \rightarrow \texttt{Signal } (a \times b)$:

\[\text{sample} \; xs \; ys = map \; (\lambda x. (x, head \; ys)) \; xs\]

The $sample$ combinator allows sampling the current value of $ys$, every time that a new value $x$ is produced on the signal $xs$. If it weren't for the mutable semantics, this combinator would have had to keep the entirety of $ys$ (beginning from its first value) in memory.
