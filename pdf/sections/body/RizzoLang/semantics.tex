...
Then as described by the update semantics, each signal is moved one-by-one from the \texttt{earlier heap} to the \texttt{now heap}. To move a signal we first check if its tail was dependent on $\mathbf{C}$ and if it were, then the tail is \textit{advanced} to obtain a new signal whose data is copied into the original signal and the original signal is moved to the the \texttt{now heap}. Otherwise, the signal is moved to the \texttt{now heap} unchanged.
...
The exception is the \texttt{Signal} constructor. Although signals, like other constructors, are just containers of values, they must be treated differently because both their creation and erasure modify the signal heap.
We therefore introduce a specialised \textbf{signal} value.
...
When a signal is later erased, its fields are decremented and the signal is removed from the heap.
