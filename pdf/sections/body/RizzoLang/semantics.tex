...
When applying the \textbf{signal} constructor with a set of arguments, each argument is evaluated and stored into a fresh memory location, just as with other constructors, but the resulting value is also inserted into the signal heap.
...
Although we treat the advance semantics as a black-box, its interface is expected to borrow any arguments passed and return incremented values, so the update semantics can safely decrement the result of calling advance, whether or not that call created a fresh signal value.
