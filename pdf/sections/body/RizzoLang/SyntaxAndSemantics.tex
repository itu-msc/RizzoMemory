...
The modal types \texttt{Later} and \texttt{Delayed} are what distinguish Rizzo from ordinary functional languages. As introduced in Section~\ref{section:frp:rizzo}, these types encode temporal properties of values.\citeauthor*{rizzo_modal_2025} introduced a set of constructors for these modal types, for which the constructors for \texttt{Later} values are:
...
In RizzoLang we have 2 types of constructors: the standard constructors for values (e.g. tuples, strings, sync and so on) and the signal constructor \texttt{::}. The semantics of the standard constructors are mostly straightforward: they allocate a new object with a reference count of 1, and the fields are initialized with the boxed values of the constructor arguments. The semantics of the signal constructor are more complex, as they must also interact with the heap structure and the reactive semantics. When a signal is constructed with \texttt{e1 :: e2}, the runtime allocates a new signal object, initializes its head with the value of \texttt{e1}, and initializes its tail with the value of \texttt{e2}. The new signal is then inserted into the heap just before the current cursor position. This ensures the signals is placed after the signal it depends on (if any), but before the next signal to be updated. Like any constructor, the new signal starts with a reference count of 1. The reference counts of the head and tail values are incremented accordingly, as the new signal now holds references to them.
...
In Rizzo the reactive semantics require that all signals are traversed when a channel produces a value.
