...
\citeauthor*{rizzo_modal_2025}~\cite{rizzo_modal_2025} introduce a set of constructors for these modal types. For the \texttt{Later} type, the constructors are:
...
\begin{itemize}
    ...
    \item $\mathsf{laterapp} \; f \; x : \texttt{Later}\,b$ is the applicative action on \texttt{Later} values. It builds a delayed computation to apply $f : \texttt{Delayed}\,(a \to b)$ to $x : \texttt{Later}\,a$ in the future time step when $x$ produces its value.
...
and for the \texttt{Delayed} type the constructors are:
\begin{itemize}
    \item $\mathsf{delay}\;e : \texttt{Delayed}\,a$ delays the expression $e : a$ to a future time step.
