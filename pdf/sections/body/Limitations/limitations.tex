...
\]
This works because, according to the reactive semantics of $\mathsf{tail}$, advancing a $\mathsf{tail}$ value returns a pointer to the signal it was given.
...
The downside is that we abandon the lazy signal creation. This means that if the signal is never used (\todo{never used? Or if the later never ticks?}), we have unnecessarily created a signal and allocated memory for it.
However, if the signal is used multiple times, we save on the redundant later applications and the associated memory allocations, which can be a significant improvement in performance and memory usage.
