...
By using new \textbf{reset} and \textbf{reuse} instructions and ownership annotations, the compiler can determine when memory of immutable data can be safely overwritten and when increment/decrement instructions can be safely omitted, and when they cannot.
...
In this section, we will be using the term \textit{RC token} (Reference Counting token).
...
\citeauthor*{ullrich_counting_2021} \cite{ullrich_counting_2021} gains their performance improvements by using a combination of ownership annotations, and new \textbf{reset} and \textbf{reuse} instructions.
The ownership annotations in \citetitle{ullrich_counting_2021} \cite{ullrich_counting_2021} distinguish between $\mathbb{O}wned$ and $\mathbb{B}orrowed$ references, and lets the compiler track which parts of the program are responsible for managing the memory of each object. This knowledge allows the compiler to determine when an object needs to be incremented or decremented, which reduces the number of reference counting operations that need to be performed. 
On the other hand, the new \textbf{reset} and \textbf{reuse} instructions allows the compiler to optimize memory allocations of new objects, by reusing memory from old objects that are no longer needed, instead of always allocating new memory for each object. This leads to performance improvements, especially in cases where many short-lived objects are created and destroyed.
...
\subsection{Purpose of ownership}

\todo{If we own a parameter, then we need to ensure that the parameter is alive as long as we need it.
If we borrow a parameter, then we assume that the caller has the responsibility to ensure that the parameter is alive as long as we need it. This means we can avoid incrementing the reference count of the parameter. This is a great performance improvement when we go down in a scope where all the functions can borrow their parameters, and thus only increment the reference count once at the top of the scope, instead of incrementing it for each function call like we would need if it was owned.
We can typicaly borrow parameters when we only need to read from them, and own the parameters when we need to write to them.}

\subsection{Examples}\label{section:beans:examples}
The following examples use a notation close to the reference counted intermediate language generated by the compiler. The instructions \textbf{inc} and \textbf{dec} increment and decrement the reference count of a variable, while \textbf{reset} and \textbf{reuse} are used to reclaim and recycle storage.
...
Assume that the function $c$ takes two parameters and consumes an RC token for each of them. Consuming an RC token for a parameter will mark the parameter as $\mathbb{O}wned$, which means that the caller is responsible for the reference count of the objects. In this case, since $y$ is passed twice to $c$, both occurrences of $y$ are treated as owned, and both will consume an RC token. The compiler must then insert an \textbf{inc} before the call to make the second owned use valid:
...
In the `then' branch, the input signal $(x :: xs)$ is not used. This makes it possible reuse the memory cell to store the new signal $x :: \mathsf{never}$, avoiding an allocation. 
