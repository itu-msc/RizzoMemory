...
By using new \textbf{reset} and \textbf{reuse} instructions in combination with ownership types, the compiler can determine when memory of immutable data can be safely overwritten and when increment/decrement instructions can be safely omitted, and when they cannot.
...
\citeauthor*{ullrich_counting_2021} \cite{ullrich_counting_2021} gains their performance improvements by using a combination of ownership types and new \textbf{reset} and \textbf{reuse} instructions.
...
\subsection{examples}
The following examples use a small notation close to the reference counted intermediate language generated by the compiler. The instructions \textbf{inc} and \textbf{dec} increment and decrement the reference count of a variable, while \textbf{reset} and \textbf{reuse} are used to reclaim and recycle storage.
...
Assume that both parameters of $c$ are owned. The call therefore consumes two RC tokens, but the variable $y$ initially contributes only one. The compiler must insert an \textbf{inc} before the call to make the second owned use valid:
...
The more interesting optimisation in \citetitle{ullrich_counting_2021} is the combination of \textbf{reset} and \textbf{reuse}. In Rizzo this matters when a combinator stops using an input signal and immediately constructs a replacement signal. Consider the function below:
...
In the `then' branch, the old signal $s$ is dead immediately after the match. This makes it possible to reset the old signal cell and reuse its storage for the new signal $x :: \mathsf{never}$.
