...
When compiling the program we enable the \texttt{--debug-info} flag, which enables the logging of debug information. Specifically, the flag enables printing of the heap every time a channel produces a value, and it will assign a unique identifier to each signal so it can be traced through a series of prints. 
