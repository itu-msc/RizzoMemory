...
To implement the desired behaviour, the program uses $clock$ and $console-input$ signals, and manipulates them with the $head$, $tail$, $map$, $filter$, $switch$, $trigger$, and $interleave$ families of signal combinators. Through these combinators, the example makes use of Rizzo's core temporal features: signal construction ($::$), sync construction, $watch$, $wait$, $tail$, and Later application ($LaterApp$). The only modal constructors not used by the example are $never$ and what we call $ostar$.
