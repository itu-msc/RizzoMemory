...
let rec expr_to_rexpr (e: Ast.expr) : Refcount.rexpr = match e with
  ...
  | EUnary (_, EVar _)  -> failwith "todo - unary to rexpr"
  | EConst _ -> failwith "todo - const to rexpr"
  | _ -> failwith "invalid expression to convert"
and expr_to_fn_body (e: Ast.expr) : Refcount.fn_body = 
  match e with
  | EConst _ -> failwith "expr_to_fn_body: todo const"
  | EVar x -> FnRet x
  | ELet (x, rhs, e') ->
...
let to_rc_intermediate_representation (p: Ast.program) : Refcount.program =
  ...
  | _ -> failwith "only top level functions right now"
  in
